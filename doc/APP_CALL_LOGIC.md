# app_main.py → app.py 调用逻辑详解

## 一、两个入口文件的关系

### 📁 文件位置
- **旧版本入口**：`app_main.py`（项目根目录）
- **新版本入口**：`src/yoloface/app.py`（模块化结构）

### 🔄 它们的关系

**重要说明**：这两个文件**不是调用关系**，而是**两个独立的入口点**，代表项目的两个版本：

1. **`app_main.py`** - 旧版本（遗留代码）
   - 直接在根目录
   - 直接导入旧的测试文件（`cv_test.py`, `yolo_test.py`等）
   - 代码结构简单，但不够模块化

2. **`src/yoloface/app.py`** - 新版本（模块化重构后）
   - 在模块化结构中
   - 使用新的模块化设计（`yoloface.detectors`, `yoloface.gui`等）
   - 代码结构清晰，易于维护
